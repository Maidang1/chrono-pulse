...
export default function EventAnalysis() {
  ...
  const stats = useMemo(() => {
    ...
    if (eventData.type === "time-tracking") {
      const totalMinutes = eventData.records.reduce(
        (acc, record) => acc + (record.durationMinutes || 0),
        0,
      );
      const longestRecord = eventData.records.reduce(
        (max, record) => Math.max(max, record.durationMinutes || 0),
        0,
      );
      const averageDuration =
        eventData.records.length > 0
          ? Math.round(totalMinutes / eventData.records.length)
          : 0;
      ...
      return {
        ...
        longestRecord,
        averageDuration,
        recordCount: eventData.records.length,
      };
    } else {
      const total = eventData.records.length;
      const completed = eventData.records.filter((r) => r.completed).length;
  ...
  const themeStyles = {
    ...
    analysisLabel:
      actualTheme === "dark"
        ? "text-[24rpx] uppercase tracking-[0.12em] text-[#cccccc] leading-[1.4]"
        : "text-[24rpx] uppercase tracking-[0.12em] text-[#4a4a4a] leading-[1.4]",
    analysisTitle:
      actualTheme === "dark"
        ? "text-[64rpx] leading-[1.25] font-bold text-[#ffffff]"
        : "text-[64rpx] leading-[1.25] font-bold text-[#1a1a1a]",
    analysisDate:
      actualTheme === "dark"
        ? "text-[24rpx] uppercase tracking-[0.12em] text-[#888888] leading-[1.4]"
        : "text-[24rpx] uppercase tracking-[0.12em] text-[#888888] leading-[1.4]",
    statCard:
      actualTheme === "dark"
    ...
    statLabel:
      actualTheme === "dark"
        ? "text-[24rpx] uppercase tracking-[0.1em] text-[#888888] leading-[1.35]"
        : "text-[24rpx] uppercase tracking-[0.1em] text-[#888888] leading-[1.35]",
    statValue:
      actualTheme === "dark"
        ? "text-[36rpx] font-bold leading-[1.2] text-[#ffffff]"
        : "text-[36rpx] font-bold leading-[1.2] text-[#1a1a1a]",
    recordsCard:
      actualTheme === "dark"
    ...
    recordTime:
      actualTheme === "dark"
        ? "text-[28rpx] font-semibold text-[#ffffff]"
        : "text-[28rpx] font-semibold text-[#1a1a1a]",
    recordNote:
      actualTheme === "dark"
        ? "text-[24rpx] text-[#888888]"
        : "text-[24rpx] text-[#888888]",
    progressBar:
      actualTheme === "dark"
    ...
    recordDuration:
      actualTheme === "dark"
        ? "text-[26rpx] font-semibold text-left text-[#ffffff]"
        : "text-[26rpx] font-semibold text-left text-[#1a1a1a]",
    emptyState:
      actualTheme === "dark"
        ? "text-[26rpx] text-[#cccccc]"
        : "text-[26rpx] text-[#4a4a4a]",
    errorState:
      actualTheme === "dark"
            ...
            {stats?.type === "time-tracking" ? (
              <View className="grid grid-cols-2 gap-[16rpx]">
                ...
                <View className={themeStyles.statCard}>
                  ...
                  <Text className={themeStyles.statValue}>
                    {stats.longestRecord
                      ? formatMinutes(stats.longestRecord)
                      : "—"}
                  </Text>
                </View>
                <View className={themeStyles.statCard}>
              ...
              <View className="grid grid-cols-2 gap-[16rpx]">
                <View className={themeStyles.statCard}>
                  <Text className={themeStyles.statLabel}>总计</Text>
                  <Text className={themeStyles.statValue}>{stats?.total}</Text>
                </View>
                <View className={themeStyles.statCard}>
                  <Text className={themeStyles.statLabel}>已完成</Text>
                  <Text className={themeStyles.statValue}>
                    {stats?.completed}
                  </Text>
                </View>
                <View className={themeStyles.statCard}>
                  <Text className={themeStyles.statLabel}>未完成</Text>
                  <Text className={themeStyles.statValue}>
                    {stats?.pending}
                  </Text>
                </View>
                <View className={themeStyles.statCard}>
                  <Text className={themeStyles.statLabel}>完成率</Text>
                  <Text className={themeStyles.statValue}>
            ...
            {eventData.records.length ? (
              eventData.type === "time-tracking" ? (
                eventData.records.map((record) => {
                  const width =
                    stats?.type === "time-tracking" && stats.longestRecord
                      ? Math.round(
                  ...
                  return (
                    <View
                      key={record.id}
                      className="grid grid-cols-1 gap-[12rpx] items-start"
                    >
                      ...
                      <Text className={themeStyles.recordDuration}>
                        {formatMinutes(record.durationMinutes || 0)}
                      </Text>
                    </View>
                  );
                })
                ...
                eventData.records.map((record) => (
                  <View key={record.id} className="flex flex-col gap-[8rpx]">
                    <View className="flex items-center gap-[12rpx]">
                      <Text className="text-[32rpx]">
                        {record.completed ? "☑" : "☐"}
                      </Text>
                      <Text
                        className={`${themeStyles.recordNote} ${record.completed ? "line-through" : ""}`}
                      >
                        {record.note}
                      </Text>
                    </View>
